...
# Maps each palette color to a context keyword. A task is painted a color when
# its title/description mentions the matching @keyword or #keyword. Empty string
...
# Endpoints
@app.get("/api/settings/contexts")
def get_contexts():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT value FROM settings WHERE key = 'contexts'")
    row = cursor.fetchone()
    conn.close()
    if row:
        # Merge over defaults so missing keys fall back gracefully.
        return {**DEFAULT_CONTEXTS, **json.loads(row["value"])}
    return DEFAULT_CONTEXTS

@app.put("/api/settings/contexts")
def update_contexts(contexts: Contexts):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO settings (key, value) VALUES ('contexts', ?) "
        "ON CONFLICT(key) DO UPDATE SET value = excluded.value",
        (contexts.model_dump_json(),)
    )
    conn.commit()
    conn.close()
    return contexts.model_dump()
